...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!targetUserId || targetUserId.length !== 24) {
      return NextResponse.json<ApiResponse>(
        { success: false, error: 'Valid target user ID is required' },
        { status: 400 }
      );
    }
    ...
    // Fix: Handle the case where updatedTargetUser might be null
    ...
    return NextResponse.json<ApiResponse>({
      ...
      data: {
        ...
        targetUser: {
          ...updatedTargetUser,
          _id: targetUserId // Use the original targetUserId since we know it exists
        }
      }
    });
  } catch (error) {
