// app/api/courses/[id]/payment/khalti-callback/route.ts
  ...
  try {
    ...
    if (!pidx || pidx.trim().length === 0) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/payment/failed?error=missing_pidx`
      );
    }
      ...
      if (pendingEnrollment && pendingEnrollment.status === 'pending') {
        ...
        if (course && user) {
          ...
          const isAlreadyEnrolled = course.students.some(
            (student: any) => student.user.toString() === user._id.toString()
          );
          ...
          return NextResponse.redirect(
            `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/courses/${course._id}/learn?enrolled=true`
          );
        }
      }
    }
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/payment/failed?error=verification_failed`
    );
  } catch (error) {
    console.error('Khalti callback error:', error);
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/payment/failed?error=internal_error`
    );
  }
}