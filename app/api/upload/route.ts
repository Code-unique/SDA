...
export async function POST(request: NextRequest) {
  try {
    ...
    const allowedTypes = {
      ...
      previewVideo: ['video/mp4', 'video/webm', 'video/ogg']
    };
    ...
    // For API route: only handle small files (4MB max)
    const maxSize = 4 * 1024 * 1024; // 4MB
    if (file.size > maxSize) {
      return NextResponse.json(
        { error: 'File too large for API upload. Use S3 direct upload for larger files.' },
        { status: 400 }
      );
    }
