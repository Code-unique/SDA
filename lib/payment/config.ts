class PaymentConfig {
  ...
  // Prevent validation during build step
  ...
  if (isRuntimeProduction) {
    ...
    if (publicKey && !publicKey.startsWith('pk_test_') && !publicKey.startsWith('pk_live_')) {
      console.warn('Stripe public key should start with "pk_test_" for development')
    }
    if (secretKey && !secretKey.startsWith('sk_test_') && !secretKey.startsWith('sk_live_')) {
      console.warn('Stripe secret key should start with "sk_test_" for development')
    }
  }
