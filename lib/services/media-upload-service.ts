...
export class MediaUploadService {
  static async uploadFiles(
    ...
    for (const file of files) {
      ...
      try {
        ...
        const cloudinaryResult: any = await uploadPostMedia(
          file,
          isImage ? 'image' : 'video',
          (progress) => {
            onProgress?.({
  ...
  static validateFileCollection(files: File[], existingCount: number = 0): {
    isValid: boolean;
    errors: string[];
    warnings: string[];
  } {
  ...
  static async validateVideoDuration(file: File): Promise<{
    isValid: boolean;
    duration?: number;
    error?: string;
  }> {
  ...
  static formatFileInfo(file: File): {
    name: string;
    size: string;
    type: string;
    isImage: boolean;
