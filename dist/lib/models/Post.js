...
const PostSchema = new Schema({
    ...
    toJSON: {
        ...
        transform: function (doc, ret) {
            ...
            // Convert likes array
            if (ret.likes) {
                ret.likes = ret.likes.map((id) => id.toString ? id.toString() : id);
            }
            // Convert saves array
            if (ret.saves) {
                ret.saves = ret.saves.map((id) => id.toString ? id.toString() : id);
            }
            // Handle comments
            if (ret.comments) {
                ret.comments = ret.comments.map((comment) => {
                    const commentObj = Object.assign(Object.assign({}, comment), { _id: comment._id ? comment._id.toString() : comment._id, user: comment.user ? {
                        ...
                        } : null, likes: comment.likes ? comment.likes.map((id) => id.toString ? id.toString() : id) : [], parentComment: comment.parentComment ? (comment.parentComment.toString ? comment.parentComment.toString() : comment.parentComment) : null });
                    return commentObj;
                });
            }
            // Add virtual counts
...
PostSchema.virtual('hasMultipleMedia').get(function () {
...
PostSchema.virtual('hasVideo').get(function () {
...
// ================== METHODS ==================
PostSchema.methods.addComment = async function (userId, text, parentCommentId) {
    const commentData = {
    ...
    if (parentCommentId) {
        ...
        const parentComment = this.comments.find((c) => c._id.equals(parentCommentId));
        if (parentComment) {
...
PostSchema.methods.updateComment = async function (commentId, text) {
    const comment = this.comments.find((c) => c._id.equals(commentId));
    if (!comment)
        return null;
...
PostSchema.methods.deleteComment = async function (commentId) {
    ...
    if (comment.parentComment) {
        const parent = this.comments.find((c) => c._id.equals(comment.parentComment));
        if (parent) {
            parent.replies = parent.replies.filter((replyId) => !replyId.equals(commentId));
        }
    }
    // Remove the comment
