export class ImageCritiqueEngine {
    ...
    extractFit(text) {
        if (/tight|fitted|form-fitting/.test(text))
            return 'fitted';
        if (/loose|oversized|baggy/.test(text))
            return 'loose';
        if (/tailored|structured/.test(text))
            return 'tailored';
        return 'standard';
    }
    extractOccasion(text) {
        if (/work|office|business/.test(text))
            return 'professional';
        if (/party|evening|gala/.test(text))
            return 'formal';
        if (/casual|everyday|street/.test(text))
            return 'casual';
        if (/wedding|ceremony/.test(text))
            return 'ceremonial';
        return 'general';
    }
    analyzeSilhouette(elements) {
    ...
    analyzeColorHarmony(elements) {
        const colors = elements.colors;
        if (colors.length === 0)
            return "Monochromatic or neutral palette offers versatility";
        if (colors.length === 1)
            return "Single color focus creates strong visual impact";
    ...
    analyzeFabricBehavior(elements) {
        const fabrics = elements.fabrics;
        if (fabrics.length === 0)
            return "Consider how fabric choice affects drape and movement";
        const hasHeavy = fabrics.some((f) => ['wool', 'denim', 'tweed'].includes(f));
        const hasLight = fabrics.some((f) => ['silk', 'chiffon', 'linen'].includes(f));
    ...
    generateOverallAssessment(elements) {
        const strengths = [];
        const improvements = [];
